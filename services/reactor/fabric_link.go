...
func (r *Reactor) startPassiveFabric(ctx context.Context, ev types.SerialSessionOpened) {
	...
	if rx == nil || tx == nil {
		log.Println(fabricLogPrefix + "missing rings")
		return
	}
	...
	if fabricConn == nil {
		_ = tr.Close()
		log.Println(fabricLogPrefix + "missing bus")
		return
	}
	...
	log.Println(fabricLogPrefix+"open ", transferMode)
	go func() {
		defer close(done)
		defer tr.Close()
	...
	log.Println(fabricLogPrefix+"opened ", transferMode)
}
...
func (r *Reactor) stopFabricLink() {
	...
	if !waitFabricDone(done, fabricStopWaitTimeout) {
		log.Println(fabricLogPrefix + "stop timeout")
	}
}
