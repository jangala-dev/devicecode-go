...
func TestDecodeChunkDataUsesFixedBuffersAcrossManyChunks(t *testing.T) {
	...
	allocs := testing.AllocsPerRun(100, func() {
		for i := 0; i < 64; i++ {
			got, errStr := s.decodeChunkDataBytes(encoded)
			if errStr != "" {
...
func TestDecodeXferChunkFastKeepsDataAsLineSlice(t *testing.T) {
	line := []byte(`{"type":"xfer_chunk","xfer_id":"fabric-xfer-1","offset":2048,"data":"YWJjZA","chunk_digest":"00000000"}`)
	msg, ok := decodeXferChunkFast(line)
	if !ok {
		t.Fatalf("decodeXferChunkFast failed")
	}
	if msg.Data != "" {
		t.Fatalf("Data string = %q, want empty fast-path string", msg.Data)
	}
	if string(msg.DataRaw) != "YWJjZA" {
		t.Fatalf("DataRaw = %q", msg.DataRaw)
	}
	line[0] = '['
	if string(msg.DataRaw) != "YWJjZA" {
		t.Fatalf("DataRaw should point at the original data span only, got %q", msg.DataRaw)
	}
}
