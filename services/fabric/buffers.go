...
type FabricBuffers struct {
	// TransportLine is used by shmring transport while assembling one line from
	// the UART RX ring. The session read loop copies completed lines into a free
	// RXLines slot before handing them to the reactor goroutine.
	TransportLine [maxLineLen]byte
